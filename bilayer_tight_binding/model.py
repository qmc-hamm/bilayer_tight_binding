...
def load_graphene_fit():
    ...
    fit = {}
    f = "/".join(bilayer_tight_binding.__file__.split("/")[:-1])+"/parameters/fit_graphene.hdf5"
...
def load_bilayer_fit():
    # Load in fits, average over k-folds
    fit = {}
    f = "/".join(bilayer_tight_binding.__file__.split("/")[:-1])+"/parameters/fit_bilayer.hdf5"
    with h5py.File(f,'r') as hdf:
